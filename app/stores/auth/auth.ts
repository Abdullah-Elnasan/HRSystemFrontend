...
import { defineStore } from 'pinia'
import type { UserMe, LoginForm, AuthResponse } from '~/types/auth'
  ...
  actions: {
    ...
    async login(credentials: LoginForm, toast: any) {
      ...
      try {
        ...
        toast.add({
          title: response.messageAr ?? 'تم تسجيل الدخول بنجاح',
          color: 'success',
        })
    ...
    async fetchUser() {
      ...
      const response = await $fetch<{ data: UserMe }>(
        `${apiBase}/api/auth/me`,
        {
          method: 'GET',
          credentials: 'include',
    ...
    // =========================
    // CHECK AUTH (NO DUPLICATES)
    // =========================

    async checkAuth(): Promise<boolean> {
      if (this.user) return true
      if (this.authChecked) return false
      ...
      this.authPromise = this.fetchUser()
        .then((user) => {
          this.authChecked = true
