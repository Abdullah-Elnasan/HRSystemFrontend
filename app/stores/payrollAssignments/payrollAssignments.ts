...
interface PayrollAssignmentsState {
...
export const usePayrollAssignmentsStore = defineStore('payrollAssignments', {
  ...
  actions: {
    ...
    async fetchAssignments(params?: Record<string, any>, signal?: AbortSignal) {
      ...
      try {
        const response = await $api<apiResponse<PayrollAssignment[]>>(
          `${config.public.apiBase}/api/payroll-assignments`,
          { query: params, signal }
        )
    ...
    async fetchAssignmentById(id: number | string) {
      ...
      try {
        const response = await $api<apiResponse<PayrollAssignment>>(
          `${config.public.apiBase}/api/payroll-assignments/${id}`
        )
        const index = this.assignments.findIndex(a => a.id === response.data.id)
