...
import { emptyWorkScheduleForm } from "~/types/workSchedule";
...
const openDrower = (payload: { title: string; row?: unknown }) => {
  ...
  if (payload.row && isWorkScheduleRow(payload.row)) {
    ...
    Object.assign(formModel, {
      ...
      is_active: Boolean(payload.row.is_active),
    });
  } else {
...
const meta = {
  class: {
    tr: (row: any) => {
      console.log(row.original.status === false);
      return row.original.is_active === false
        ? "bg-error/10 bg-white dark:bg-gray-900 shadow-sm ring-1 ring-default/10 rounded-lg transition-shadow"
...
const statusMap: Record<string, { label: string; color: string }> = {
  ...
  flexible: { label: "مرن", color: "error" },
