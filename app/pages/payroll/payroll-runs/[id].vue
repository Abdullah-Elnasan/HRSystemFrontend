...
const payrollRunId = computed(() => route.params.id as string)
...
// ─── canEdit: بناءً على أول عنصر ─────────────────────
const canEditRow = computed(() =>
  enhancedItems.value[0]?.status === 'pending'
)
...
<template>
  ...
  <AppTable
    ...
    :actions="{
      ...
      view:   false,
      delete: false,
      edit:   canEditRow ? { label: 'تعديل يدوي' } : false,
    }"
    :btn-create="false"
